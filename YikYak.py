		...
		while True:
			...
			# Show Recent Yak Upvotes
				...
				if len(choice) > 2:
					...
					textToFind = input("Enter text to find and upvote: \n")	
				
				yaklist = remoteyakker.get_yaks()
				index = 0
				while(textToFind in yaklist[index].message == False):
					index = index + 1	
				print("Yak is: " + yaklist[index].message + "\n")
				...
				for i in range(20):
					line = fp.readline()
					print("New user id is: " + line[:-1])
					remoteyakker = setUserID(remoteyakker.location, line[:-1])
					remoteyakker.upvote_yak(yaklist[index].message_id)
				

			# Get Yakarma
			elif choice[0].upper() == 'X':
				while remoteyakker.get_yakarma() < 300000:
					# get random location
					currentlatitude = random.uniform(30, 50)
					currentlongitude = random.uniform(-120, -70)
					coordlocation = pk.Location(currentlatitude, currentlongitude)
					...
					for i in range(len(yaklist)):
						upvoted = remoteyakker.upvote_yak(yaklist[i].message_id)

			elif choice[0].upper() == 'Z':
				if len(choice) > 2:
					commentNum = choice[2:]
				else:
					commentNum = input("Enter comment number: \n")
				currentlist = remoteyakker.get_yaks()
				#print(currentlist[int(commentNum)].getPoster().getUserID())
				print(currentlist[int(commentNum)].print_yak())
			...
			# Upvote Yak
				...
				if len(currentlist) > 0:
					...
					for i in range(voteYakNum):
						#upvoted = remoteyakker.upvote_yak(currentlist[voteYakNum-1].message_id)
						upvoted = remoteyakker.upvote_yak(currentlist[i].message_id)
